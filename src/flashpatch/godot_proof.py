...
SCHEMA = "flashpatch-godot-proof-v1"
VERIFY_SCHEMA = "flashpatch-godot-proof-verification-v1"


class GodotProofError(ValueError):
    """A Godot-proof artifact failed its public verification contract."""
...
def _load_frames(path: Path) -> np.ndarray:
    ...
    if frames.ndim != 4 or frames.shape[-1] != 3 or frames.dtype != np.uint8:
        raise GodotProofError("renderer artifact must contain uint8 RGB frames")
    return frames


def _write_rgb(path: Path, frame: np.ndarray) -> None:
    if not cv2.imwrite(str(path), cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)):
        raise GodotProofError(f"could not write image: {path.name}")
...
def _minimal_single_line_diff(diff: str, source: str, source_line: int) -> str:
    ...
    if len(deleted) != 1 or len(added) != 1:
        raise GodotProofError("Godot proof patch must contain exactly one changed source line")
    return "\n".join([
        f"--- a/{source}",
        f"+++ b/{source}",
...
def run_godot_proof(
    ...
    if len(contract.candidates) != 1:
        raise GodotProofError("Godot proof requires exactly one declared patch candidate")
    ...
    if not isinstance(factual, dict) or not isinstance(attribution, dict):
        raise GodotProofError("PASS engine receipt omitted factual or attribution evidence")
    ...
    if not isinstance(hazard_indices, list) or not hazard_indices:
        raise GodotProofError("PASS engine receipt omitted hazard frame indices")
    valid_indices = [
        index for index in hazard_indices
        if isinstance(index, int) and 0 <= index < min(len(factual_frames), len(candidate_frames))
    ]
    if not valid_indices:
        raise GodotProofError("PASS engine receipt has no usable hazard frame index")
    ...
    if not isinstance(source_line, int) or source_line < 1:
        raise GodotProofError("PASS engine receipt omitted a valid source line")
    ...
    if not isinstance(factual_capture, dict) or not isinstance(candidate_capture, dict) or not isinstance(runtime, dict):
        raise GodotProofError("PASS engine receipt omitted renderer or runtime attribution")
    artifacts = {
...
def _artifact_path(root: Path, relative: str) -> Path:
    ...
    if root.resolve() not in candidate.parents or not candidate.is_file():
        raise GodotProofError(f"artifact path is missing or escapes receipt root: {relative}")
    return candidate


def verify_godot_proof(receipt_path: Path | str) -> dict[str, object]:
    path = Path(receipt_path).resolve()
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or payload.get("schema") != SCHEMA:
        raise GodotProofError("Godot proof receipt schema is invalid")
    claimed = payload.get("receipt_sha256")
    unhashed = dict(payload)
    unhashed.pop("receipt_sha256", None)
    if claimed != _sha256_bytes(_canonical(unhashed)):
        raise GodotProofError("Godot proof receipt hash does not match")
    if payload.get("verdict") != "PASS":
        raise GodotProofError("Godot proof receipt is not PASS")
    artifacts = payload.get("artifacts")
    if not isinstance(artifacts, dict) or set(artifacts) != {
        "before.png", "after.png", "comparison.png", "patch.diff", "engine-receipt.json"
    }:
        raise GodotProofError("Godot proof artifact manifest is incomplete")
    for relative, expected_hash in artifacts.items():
        artifact = _artifact_path(path.parent, relative)
        if _sha256_file(artifact) != expected_hash:
            raise GodotProofError(f"Godot proof artifact hash mismatch: {relative}")
    ...
    if not all(isinstance(item, dict) for item in (hazard, replay, patch, isolation)):
        raise GodotProofError("Godot proof receipt omitted a required evidence section")
    ...
    if not isinstance(before, dict) or not isinstance(after, dict) or not isinstance(threshold, (int, float)):
        raise GodotProofError("Godot proof hazard evidence is invalid")
    if before.get("hazardous") is not True or after.get("hazardous") is not False:
        raise GodotProofError("Godot proof does not prove hazard removal")
    if not isinstance(before.get("max_risk"), (int, float)) or float(before["max_risk"]) < float(threshold):
        raise GodotProofError("Godot proof factual risk is below its threshold")
    if not isinstance(after.get("max_risk"), (int, float)) or float(after["max_risk"]) >= float(threshold):
        raise GodotProofError("Godot proof patched risk is not below its threshold")
    if patch.get("changed_source_assignments") != 1:
        raise GodotProofError("Godot proof patch is not a single source assignment")
...
def format_godot_proof(receipt: dict[str, Any], output: Path | str) -> str:
